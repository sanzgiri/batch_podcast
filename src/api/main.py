...
from src.lib.exceptions import LLMError, ProcessingError, TTSError, ValidationError
...
def add_error_handlers(app: FastAPI):
    ...
    async def validation_error_handler(_request: Request, exc: ValidationError):
        ...
        return JSONResponse(
            status_code=400,
            content={"error": "Validation Error", "message": str(exc), "type": "validation_error"},
        )
    ...
    async def processing_error_handler(_request: Request, exc: ProcessingError):
        ...
        return JSONResponse(
            status_code=422,
            content={"error": "Processing Error", "message": str(exc), "type": "processing_error"},
        )
...
def add_middleware(app: FastAPI):
    ...
    async def logging_middleware(request: Request, call_next):
        ...
        try:
            ...
            process_time = time.time() - start_time
            logger.error(
                f"Request failed: {request.method} {request.url} ({process_time:.3f}s) - {e}"
            )
            raise
