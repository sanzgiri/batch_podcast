...
class NewsletterURLRequest(BaseModel):
    ...
    style: str = Field(
        "conversational", description="Summary style: conversational, formal, casual"
    )
    target_length: str = Field("medium", description="Target length: short, medium, long")
    voice: str | None = Field(None, description="TTS voice (provider-specific)")
    ...
    focus_areas: list[str] | None = Field(None, description="Topics to emphasize")
...
async def submit_newsletter_from_url(
    ...
    try:
        ...
        background_tasks.add_task(
            _process_newsletter_url, str(request.url), request.user_id, processing_options
        )
    ...
    try:
        ...
        async with NewsletterProcessor(config) as processor:
            newsletter = await processor.process_newsletter_from_url(
                url=url, user_id=user_id, processing_options=processing_options
            )
