...
class ContentExtractor:
    ...
    async def __aenter__(self):
        ...
        self.session = aiohttp.ClientSession(
            ...
            headers={"User-Agent": "Newsletter-Podcast-Generator/1.0 (+https://example.com)"},
        )
        return self
    ...
    async def extract_from_url(self, url: str) -> ExtractedContent:
        ...
        try:
            async with self.session.get(url) as response:
                ...
                extracted = await self._extract_from_html(html_content, source_url=url)
            ...
            raise ContentExtractionError(f"Failed to fetch content from {url}: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error extracting from URL {url}: {e}")
            raise ContentExtractionError(f"Content extraction failed: {e}") from e

    async def extract_from_text(
        self, content: str, content_type: str = "text", title: str | None = None
    ) -> ExtractedContent:
        """
    ...
    async def _extract_from_html(
        self, html_content: str, source_url: str | None = None, title: str | None = None
    ) -> ExtractedContent:
        """Extract content from HTML."""
        try:
            ...
            raise ContentExtractionError(f"Failed to parse HTML content: {e}") from e

    async def _extract_from_markdown(
        self, markdown_content: str, title: str | None = None
    ) -> ExtractedContent:
        ...
        try:
            ...
            raise ContentExtractionError(f"Failed to parse Markdown content: {e}") from e
    ...
    async def _extract_from_plain_text(
        ...
        try:
            ...
            raise ContentExtractionError(f"Failed to parse plain text content: {e}") from e
        ...
        if self.remove_ads:
            ...
            for pattern in ad_patterns:
                ...
                for element in soup.find_all(
                    class_=lambda x, pattern=pattern: x and pattern in x.lower()
                ):
                    element.decompose()

                # Remove by id
                for element in soup.find_all(
                    id=lambda x, pattern=pattern: x and pattern in x.lower()
                ):
                    element.decompose()
    ...
    def _validate_content_length(self, content: str) -> None:
        ...
        if word_count < self.min_content_length:
            raise ValidationError(
                f"Content too short: {word_count} words (minimum: {self.min_content_length})"
            )
